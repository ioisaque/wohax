...
function hotkeys_onkeyup(Key)  
  ...
  if Key == VK_END then
	...
	PvESetRoomPrivateStatus(not PvEGetRoomPrivateStatus())
	WFXPrint(tostring(PvEGetRoomPrivateStatus()))
	WFXPrint(tostring(PvEGetReadyStatus()))
  end
...
function hotkeys_draw()
  if canShowHint then
	...
	if notfications_enabled then myDrawText(font, NOT_COLOR, 190, 210, 20, 10, string.format("Mira do aimbot modificada para %s!", s)) end
